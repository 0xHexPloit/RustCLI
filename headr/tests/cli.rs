...
fn generate_bad_file() -> String {
    loop {
        let filename = generate_random_string();
...
fn dies_bad_bytes() -> TestResult {
    let bad_string = generate_random_string();
    let expected = format!("invalid value '{}' for '--bytes <NUM>'", bad_string);
    Command::cargo_bin(PRG)?
        .args(&["-c", &bad_string, EMPTY])
        .assert()
        .failure()
...
fn dies_bad_lines() -> TestResult {
    let bad_string = generate_random_string();
    let expected = format!("invalid value '{}' for '--lines <NUM>'", bad_string);
    Command::cargo_bin(PRG)?
        .args(&["-n", &bad_string, EMPTY])
        .assert()
        .failure()
        .stderr(predicates::str::contains(expected));
    Ok(())
...
fn dies_bytes_and_lines() -> TestResult {
    let expected = "the argument '--bytes <NUM>' cannot be used with '--lines <NUM>'";
    Command::cargo_bin(PRG)?
        .args(&["-c", "1", "-n", "2", EMPTY])
        .assert()
        .failure()
...
fn skip_bad_file() -> TestResult {
    let bad_filename = generate_bad_file();
    let expected  = format!("{}: .* [(]os error 2[)]", bad_filename);
    Command::cargo_bin(PRG)?
        .arg(bad_filename)
        .assert()
        .success()
